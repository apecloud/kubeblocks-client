...
/**
 * Switchover defines how to proactively switch the current leader to a new replica to minimize the impact on availability. This action is typically invoked when the leader is about to become unavailable due to events, such as: - switchover - stop - restart - scale-in Dedicated env vars for the action: - KB_SWITCHOVER_CANDIDATE_NAME: The name of the new candidate replica&#39;s Pod. It may be empty. - KB_SWITCHOVER_CANDIDATE_FQDN: The FQDN of the new candidate replica. It may be empty. - KB_LEADER_POD_IP: The IP address of the original leader&#39;s Pod before switchover. - KB_LEADER_POD_NAME: The name of the original leader&#39;s Pod before switchover. - KB_LEADER_POD_FQDN: The FQDN of the original leader&#39;s Pod before switchover. The env vars with following prefix are deprecated and will be removed in the future: - KB_REPLICATION_PRIMARY_POD_: The prefix of the environment variables of the original primary&#39;s Pod before switchover. - KB_CONSENSUS_LEADER_POD_: The prefix of the environment variables of the original leader&#39;s Pod before switchover. Cannot be updated.
 */
...
public class V1alpha1ComponentDefinitionSpecLifecycleActionsSwitchover {
  ...
  */
  public static V1alpha1ComponentDefinitionSpecLifecycleActionsSwitchover fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, V1alpha1ComponentDefinitionSpecLifecycleActionsSwitchover.class);
  }
